...
function System_pythonSetup()
{
    ...
    pip install --break-system-packages -r /var/www/aaa/aaa/pip.requirements # pip install requirements.
}



function System_ldapTlsSetup()
{
    # Resolve the host name in the certificate of the ad server.
    echo '10.0.111.110    ad.lab.local' >> /etc/hosts

    # Script to get the ca certificate from the samba AD server.
    # Needed to use ldaps protocol to connect to the samba AD as ldap server. 
    cp -f /vagrant/aaa/usr/bin/get_ad_ca.sh /usr/bin/get_ad_ca.sh
    chmod 755 /usr/bin/get_ad_ca.sh

    # Script to generate the /var/www/aaa/aaa/identityProvider/ad_conf.py file to authenticate users using AD as ldap server.
    cp -f /vagrant/aaa/usr/bin/ad_conf_generator.sh /usr/bin/ad_conf_generator.sh
    chmod 755 /usr/bin/ad_conf_generator.sh

    # Add an alias to check the ldap clear and ssl connection.
    newAlias="alias Ldap_smball='ldapsearch -H ldap://ad.lab.local:389 -D \"lab\\Administrator\" -w Password01 -b \"dc=lab,dc=local\" \"(cn=*)\"'"
    echo $newAlias >> /root/.bashrc
    newAlias="alias sLdap_smball='ldapsearch -H ldaps://ad.lab.local:636 -D \"lab\\Administrator\" -w Password01 -b \"dc=lab,dc=local\" \"(cn=*)\"'"
    echo $newAlias >> /root/.bashrc
}
...
{
    printf "\n* Setting up MariaDB: user sso...\n"

    databaseUserPassword=$1

    # Enable general log and log error via syslog.
    cp -f /vagrant/aaa/etc/mysql/mariadb.conf.d/51-mariadb.cnf /etc/mysql/mariadb.conf.d
...
function System_apacheSetup()
{
    ...
    # /var/www/aaa is mounted by Vagrant (share), here lays the Django stub project.
    if [ ! -d /var/www/aaa ]; then
        echo "/var/www/aaa does not exist, check your Vagrant setup."
        exit 1
    fi
    ...
    sed -i "s/\$cfg\['Servers'\]\[\$i\]\['user'\].*/\$cfg\['Servers'\]\[\$i\]\['user'\] = 'sso';/g" /var/www/myadmin/libraries/config.default.php
    ...
    cp -f /vagrant/aaa/etc/apache2/sites-available/001-django.conf /etc/apache2/sites-available/001-django.conf
    chmod 644 /etc/apache2/sites-available/001-django.conf

    # Setup the phpMyAdmin virtual host on port 8300.
    cp -f /vagrant/aaa/etc/apache2/sites-available/001-mysql.conf /etc/apache2/sites-available/001-mysql.conf
...
System_pipInstallDaemon_aaa()
{
    ...
    # pip install service.
    cp -f /vagrant/aaa/etc/systemd/system/pip_install_aaa.service /etc/systemd/system/pip_install_aaa.service
    chmod 644 /etc/systemd/system/pip_install_aaa.service

    # Watchdog service: monitor folder for changes.
    cp -f /vagrant/aaa/etc/systemd/system/pip_install_aaa.path /etc/systemd/system/pip_install_aaa.path
    chmod 644 /etc/systemd/system/pip_install_aaa.path

    systemctl daemon-reload
    systemctl enable systemd-networkd.service systemd-networkd-wait-online.service

    systemctl enable pip_install_aaa.service
    systemctl enable pip_install_aaa.path
    systemctl stop pip_install_aaa.path
    systemctl start pip_install_aaa.path
}
...
System_consulAgentInstall()
{
    ...
    # Expose Consul sso service.
    cp -f /vagrant/aaa/etc/consul.d/sso.json /etc/consul.d/sso.json
    chmod 644 /etc/consul.d/sso.json

    # Setup a Systemd Consul service unit.
    # Consul will bind to the source IP address which has route to Consul server agent.
    cp -f /vagrant/aaa/usr/bin/consul.sh /usr/bin/consul.sh
    chmod 755 /usr/bin/consul.sh

    cp -f /vagrant/aaa/etc/systemd/system/consul.service /etc/systemd/system/consul.service
...
System_syslogngInstall()
{
    ...
    cp -f /vagrant/aaa/etc/syslog-ng/conf.d/*conf /etc/syslog-ng/conf.d/
...
System_about() {
    ...
    echo "\"Version\": \"`cat /var/www/aaa/CONTAINER-DEBIAN-PKG/DEBIAN-PKG/deb.release`\"," >> $workingFolderPath/var/www/aaa/doc/about.json
    cd /var/www/aaa
