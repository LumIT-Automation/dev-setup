...
System_mariadbRestore()
{
    ...
    mysql api < /var/www/api/netscaler/sql/netscaler.schema.sql
    ...
    if [ -f /var/www/api/netscaler/sql/netscaler.data-development.sql ]; then
        mysql api < /var/www/api/netscaler/sql/netscaler.data-development.sql
    fi
}
...
System_sqliteDbRestore()
{
    ...
    if [ -f /var/lib/netscaler.sqlite ]; then
        rm -f /var/lib/netscaler.sqlite
    fi

    # Importing MysQL dump into sqlite database file.
    bash /var/www/api/mysql2sqlite.sh api | sqlite3 /var/lib/netscaler.sqlite

    if [ $? -eq 0 ]; then
        chown www-data:www-data /var/lib/netscaler.sqlite
    fi
}
