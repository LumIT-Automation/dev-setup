...
function System_mariadbSetup()
{
    ...
    cp -f /vagrant/api-zscaler/etc/mysql/mariadb.conf.d/51-mariadb.cnf /etc/mysql/mariadb.conf.d
    cp -f /vagrant/api-zscaler/etc/mysql/mariadb.conf.d/99-log.cnf /etc/mysql/mariadb.conf.d
...
function System_apacheSetup()
{
    ...
    cp /var/www/myadmin/libraries/config.default.php /var/www/myadmin/config.inc.php
    sed -i "s/\$cfg\['Servers'\]\[\$i\]\['auth_type'\].*/\$cfg\['Servers'\]\[\$i\]\['auth_type'\] = 'config';/g" /var/www/myadmin/config.inc.php
    sed -i "s/\$cfg\['Servers'\]\[\$i\]\['user'\].*/\$cfg\['Servers'\]\[\$i\]\['user'\] = 'api';/g" /var/www/myadmin/config.inc.php
    sed -i "s/\$cfg\['Servers'\]\[\$i\]\['password'\].*/\$cfg\['Servers'\]\[\$i\]\['password'\] = '$2';/g" /var/www/myadmin/config.inc.php
    ...
    cp -f /vagrant/api-zscaler/etc/apache2/sites-available/001-django.conf /etc/apache2/sites-available/001-django.conf
    chmod 644 /etc/apache2/sites-available/001-django.conf

    # Setup the phpMyAdmin virtual host on port 8300.
    cp -f /vagrant/api-zscaler/etc/apache2/sites-available/001-mysql.conf /etc/apache2/sites-available/001-mysql.conf
...
System_consulAgentInstall()
{
    ...
    cp -f /vagrant/api-zscaler/etc/consul.d/api.json /etc/consul.d/api.json
    ...
    cp -f /vagrant/api-zscaler/usr/bin/consul.sh /usr/bin/consul.sh
    chmod 755 /usr/bin/consul.sh

    cp -f /vagrant/api-zscaler/etc/systemd/system/consul.service /etc/systemd/system/consul.service
...
System_syslogngInstall()
{
    ...
    cp -f /vagrant/api-zscaler/etc/syslog-ng/conf.d/*conf /etc/syslog-ng/conf.d/
...
System_pipInstallDaemon_api()
{
    ...
    # pip install service.
    cp -f /vagrant/api-zscaler/etc/systemd/system/pip_install_api.service /etc/systemd/system/pip_install_api.service
    chmod 644 /etc/systemd/system/pip_install_api.service

    # Watchdog service: monitor folder for changes.
    cp -f /vagrant/api-zscaler/etc/systemd/system/pip_install_api.path /etc/systemd/system/pip_install_api.path
