...
System_syslogngInstall()
{
    ...
    cp -f /vagrant/uifng/etc/syslog-ng/conf.d/*conf /etc/syslog-ng/conf.d/
...
function System_consulAgentInstall()
{
    ...
    cp -f /vagrant/uifng/etc/consul.d/ui-frontend-ng.json /etc/consul.d/ui-frontend-ng.json
    chmod 644 /etc/consul.d/ui-frontend-ng.json

    # Setup a Systemd Consul service unit.
    # Consul will bind to the source IP address which has route to Consul server agent.
    cp -f /vagrant/uifng/usr/bin/consul.sh /usr/bin/consul.sh
    chmod 755 /usr/bin/consul.sh

    cp -f /vagrant/uifng/etc/systemd/system/consul.service /etc/systemd/system/consul.service
