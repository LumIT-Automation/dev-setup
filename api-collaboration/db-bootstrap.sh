...
System_mariadbRestore()
{
    ...
    mysql api < /var/www/api/collaboration/sql/collaboration.schema.sql
    mysql api < /var/www/api/collaboration/sql/collaboration.data.sql
    ...
    mysql -e 'CREATE DATABASE `phone` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci COMMENT ='"'"'pkgVersion='${pkgVer}' commit='${commit}"'"';'
    mysql -e "GRANT USAGE ON *.* TO 'api'@'%' REQUIRE NONE WITH MAX_QUERIES_PER_HOUR 0 MAX_CONNECTIONS_PER_HOUR 0 MAX_UPDATES_PER_HOUR 0 MAX_USER_CONNECTIONS 0;"
    mysql -e "GRANT ALL privileges ON *.* TO 'api'@'%';"
...
System_sqliteDbRestore()
{
    ...
    if [ -f /var/lib/collaboration.sqlite ]; then
        rm -f /var/lib/collaboration.sqlite
    fi

    # Importing MysQL dump into sqlite database file.
    bash /var/www/api/mysql2sqlite.sh api | sqlite3 /var/lib/collaboration.sqlite

    if [ $? -eq 0 ]; then
        chown www-data:www-data /var/lib/collaboration.sqlite
    fi
}
